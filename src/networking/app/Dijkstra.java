 ...
 */
	public static void computePaths(Node source){
		...
		while(!queue.isEmpty()){
			...
			for(Node v: u.adjList){
				...
				//Node v = e.target;
				double weight = Math.sqrt(Math.pow(u.location.x-v.location.x, 2)+Math.pow(u.location.y-v.location.y,2))*0.05;
                                
				//relax(u,v,weight)
				double distanceFromU = u.distance+weight;
				if(distanceFromU<v.distance){

					/*remove v from queue for updating 
					the shortestDistance value*/
					queue.remove(v);
					v.distance = distanceFromU;
					v.parent = u;
					queue.add(v);
	...
	public static List<Node> getShortestPathTo(Node target){
        ...
        public static void printShortestPath(Node target) {
            List<Node> nodes = getShortestPathTo(target);
            for(Node n : nodes) {
